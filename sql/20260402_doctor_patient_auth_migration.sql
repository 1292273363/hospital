...
-- 3) 若你的库中已有同名列，ALTER 会失败；请先手动确认或拆分执行。
...
ALTER TABLE `patient_record`
  ADD COLUMN `uploader_patient_id` BIGINT NULL COMMENT '上传人患者ID（引用patient.id）' AFTER `uploader_user_id`;

ALTER TABLE `patient_record`
  ADD KEY `idx_uploader_patient_id` (`uploader_patient_id`);

-- =========================
-- 3) visit_record 增加 patient_id / doctor_id
-- =========================
ALTER TABLE `visit_record`
  ADD COLUMN `patient_id` BIGINT NULL COMMENT '患者ID（引用patient.id）' AFTER `patient_user_id`;

ALTER TABLE `visit_record`
  ADD COLUMN `doctor_id` BIGINT NULL COMMENT '医生ID（引用doctor.id）' AFTER `doctor_level`;

ALTER TABLE `visit_record`
  ADD KEY `idx_visit_patient_id` (`patient_id`);

ALTER TABLE `visit_record`
  ADD KEY `idx_visit_doctor_id` (`doctor_id`);
...
ALTER TABLE `online_consultation`
  ADD COLUMN `doctor_id` BIGINT NULL COMMENT '医生ID（引用doctor.id）' AFTER `patient_id`;

ALTER TABLE `online_consultation`
  ADD KEY `idx_consult_patient_id` (`patient_id`);

ALTER TABLE `online_consultation`
  ADD KEY `idx_consult_doctor_id` (`doctor_id`);
...
ON DUPLICATE KEY UPDATE
  ...
  `last_login_time` = COALESCE(VALUES(`last_login_time`), `last_login_time`);
